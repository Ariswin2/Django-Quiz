...
def save_quiz_view(request, pk):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        # Remove CSRF token from the POST data
        data_.pop('csrfmiddlewaretoken', None)

        # Gather all the questions from the POST data
        for k in data_.keys():
            print("keys ", k)
        ...
        # Process each question to check answers
        for q in questions:
            a_selected = request.POST.get(q.text)

            if a_selected:  # Ensure the answer is not an empty string
                print(a_selected)
